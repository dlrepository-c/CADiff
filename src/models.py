...
class TimeAwareAttentionFusion(nn.Module):
    def __init__(self, temp, d_model):
        super().__init__()
    ...
    def forward(self, c, s, t, T):
        ...
        attn = F.softmax(logits/self.temp , dim=-1)
        fused = attn[:, 0:1] * c + attn[:, 1:2] * s
        return attn, fused


class CADiff(torch.nn.Module):  # Conditional Diffusion Recommender Model
    def __init__(self, dataset: Dataset, device, args):
        ...
        self.fusion_model = TimeAwareAttentionFusion(self.args.temperature, d_model=64)
        ## xt
    ...
    def forward_bpr(self, users, histories, lengths, pos_items):
        ...
        L_user = pos_user_loss+ neg_user_loss
        ...
        L_mse = torch.mean(mse * weight)

        return  L_user*self.args.lambda_user_loss,L_rec*self.args.lambda_rec_loss, L_mse*self.args.lambda_mse_loss
